 ...
 * Datos de prueba.
 *
 * FIX PopulateDB line 30: variable "ve" (Vehiculos) eliminada -- no se usaba.
 *
 * REGLAS CRITICAS:
 *   Admin a = new Admin();
 *   a.setUser("admin"); a.setEmail("..."); a.setPassword(encode("..."));
 *   actorRepository.save(a);    <- actorRepository, NO usuarioRepository
 *
 *   oferta.setCategoria(catObjeto);   <- objeto Categoria, NUNCA String
 */
@Component
public class PopulateDB implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        // FIX: "ve" (Vehiculos) eliminada -- no se usaba en productos ni ofertas
        cat("Vehiculos",      "vehiculos",     "directions_car", "#1976D2", null, 4);
        ...
        // ---- ADMIN (Actor, NO Usuario) -------------------------------------
        Admin adminActor = new Admin();
        adminActor.setUser("admin");
        adminActor.setEmail("admin@nexus.test");
        adminActor.setPassword(passwordEncoder.encode("admin2026!"));
        adminActor.setCuentaVerificada(true);
        adminActor.setNivelAcceso(3);
        actorRepository.save(adminActor);

        // ---- PRODUCTOS (setCategoria recibe objeto, NUNCA String) ----------
        Producto p1 = new Producto("iPhone 14 Pro 128GB", "Perfecto estado.", 750.0, TipoOferta.VENTA, carlos,
        ...
        Producto p2 = new Producto("Nike Air Max 90 T42", "Casi nuevas.", 80.0, TipoOferta.VENTA, carlos,
        ...
        Producto p3 = new Producto("Monitor LG 27GP850 165Hz", "6 meses.", 290.0, TipoOferta.VENTA, maria,
        ...
        Producto p4 = new Producto("PS5 + 3 Juegos", "Todo perfecto.", 450.0, TipoOferta.VENTA, pedro,
        ...
        Producto p5 = new Producto("Sony WH-1000XM5", "4 meses de uso.", 220.0, TipoOferta.VENTA, maria,
        ...
        // ---- OFERTAS -------------------------------------------------------
        Oferta o1 = o("AirPods Pro 2a Gen - Minimo historico", "MagSafe USB-C.",
            199.0, 279.0, "Amazon", "https://amazon.es", au, carlos,
            "https://images.unsplash.com/photo-1600294037681-c80b4cb5b434?w=800", 42, 3, 1200, 87);
        Oferta o2 = o("Roomba i5+ - El Corte Ingles", "Vaciado automatico.",
            299.0, 549.0, "El Corte Ingles", "https://elcorteingles.es", ho, maria,
            "https://images.unsplash.com/photo-1589782431773-c7b9ad4c37b2?w=800", 28, 1, 890, 44);
        Oferta o3 = o("Windows 11 Pro OEM por 9.99", "Key digital.",
            9.99, 145.0, "Kinguin", "https://kinguin.net", sw, carlos,
            "https://images.unsplash.com/photo-1587202372775-e229f172b9d7?w=800", 157, 12, 4500, 320);
        Oferta o4 = o("Xiaomi Redmi Note 13 Pro 256GB 5G", "200MP.",
            249.0, 399.0, "MediaMarkt", "https://mediamarkt.es", mo, maria,
            "https://images.unsplash.com/photo-1591337676887-a217a6970a8a?w=800", 89, 5, 2100, 143);
        Oferta o5 = o("PS Plus Essential 12m por 39.99", "Codigo digital.",
            39.99, 71.99, "PlayStation Store", "https://store.playstation.com", jg, pedro,
            "https://images.unsplash.com/photo-1606813907291-d86efa9b94db?w=800", 63, 2, 1800, 95);
        Oferta o6 = o("Samsung QLED 55\" 4K 2024", "120Hz.",
            499.0, 799.0, "PcComponentes", "https://pccomponentes.com", tv, carlos,
            "https://images.unsplash.com/photo-1593359677879-a4bb92f4834c?w=800", 35, 4, 1100, 72);

        // ---- SPARKVOTES ----------------------------------------------------
        sparkVotoRepository.save(new SparkVoto(maria, o1, true));
        sparkVotoRepository.save(new SparkVoto(pedro, o1, true));
        sparkVotoRepository.save(new SparkVoto(carlos, o2, true));
        sparkVotoRepository.save(new SparkVoto(pedro, o2, true));
        sparkVotoRepository.save(new SparkVoto(maria, o3, true));
        sparkVotoRepository.save(new SparkVoto(pedro, o3, true));
        sparkVotoRepository.save(new SparkVoto(carlos, o4, true));
        sparkVotoRepository.save(new SparkVoto(carlos, o5, true));
    ...
    private Categoria cat(String nombre, String slug, String icono, String color, Categoria parent, int orden) {
        return categoriaRepository.findBySlug(slug).orElseGet(() -> {
            ...
            c.setColor(color); c.setOrden(orden); c.setActiva(true);
    ...
    private Usuario u(String user, String email, String bio, String ubicacion) {
        ...
        u.setUser(user); u.setEmail(email);
    ...
    private Oferta o(String titulo, String desc, double precio, double orig,
                     String tienda, String url, Categoria cat, Actor actor,
                     String img, int sparks, int drips, int vistas, int comp) {
        Oferta of = new Oferta();
        of.setTitulo(titulo); of.setDescripcion(desc);
        of.setPrecioOferta(precio); of.setPrecioOriginal(orig);
        of.setTienda(tienda); of.setUrlOferta(url);
        of.setCategoria(cat); of.setActor(actor); of.setImagenPrincipal(img);
        of.setSparkCount(sparks); of.setDripCount(drips);
        of.setNumeroVistas(vistas); of.setNumeroCompartidos(comp);
        of.setEsActiva(true);
        of.setFechaPublicacion(LocalDateTime.now().minusHours((long)(Math.random() * 72)));
        of.actualizarBadge();
